...
웹 프론트엔드 없이 로컬 파일로 초안 초벌을 만들 수 있습니다. 민원 본문으로 유사 의결례를 찾고, 로컬 원문 JSON에서 detail/citation pack을 모아 `drafts/sessions/<sessionId>/` 아래에 저장합니다.
...
```bash
echo "민원 본문" | node scripts/generate-acr-draft.mjs --format md --top 5
```

생성 파일:

- `evidence.json`: 검색 결과, 원문 발췌, 로컬 검증 상태
- `draft.md`: 개인정보/법률자문 면책이 포함된 의결서 초안 초벌
- `session.json`: 세션 ID, 생성 시각, 파일 경로

현재 공식조회 어댑터는 외부 패키지를 설치하지 않고 로컬 권익위 JSON 원문 확인을 `officialLookupStatus=verified/local_source`로 기록합니다. 향후 공식 법령/결정문 조회 연결은 `connectOfficialLookupAdapter({ complaintText, decisions })` 경계에서 교체합니다.

### 점검용 스모크

```bash
node scripts/smoke-acr-semantic.mjs
node scripts/smoke-acr-draft-workflow.mjs
```
