...
import { getGeminiApiKey } from "./gemini-embed.mjs";
import {
  loadSemanticIndex,
    ...
    inputSchema: {
      ...
      properties: {
        ...
        index_path: {
          type: "string",
          description:
            "index.json 경로(선택, 기본 data/acr-decisions/semantic/index.json 상대 또는 절대)",
        },
...
async function getDecisionDetail(args) {
  ...
  return {
    ...
    file,
    header,
    reason_excerpt: clipReason(의결서["이유"], includeReason, maxReason),
  };
}
...
export async function handleJsonRpcMessage(msg) {
  ...
  try {
    ...
    return jsonRpcError(id, -32601, `Method not found: ${m.method}`);
  } catch (e) {
    const msgText = e instanceof Error ? e.message : String(e);
    return jsonRpcError(id, -32603, msgText || "Internal error");
  }
}
